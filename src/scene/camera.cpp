...
#include "../utils/ray.h"
void Camera::set_pixel_size(int hsize, int vsize, float field_of_view) {
    float half_view = tanf(field_of_view / 2);
...
Camera::Camera(int hsize, int vsize, float field_of_view, Matrix transform) : hsize(hsize), vsize(vsize), field_of_view(field_of_view), transform(transform) {
    set_pixel_size(hsize, vsize, field_of_view);
}
Camera::Camera(int hsize, int vsize, float field_of_view) : hsize(hsize), vsize(vsize), field_of_view(field_of_view), transform(IDENTITY_MATRIX) {
    set_pixel_size(hsize, vsize, field_of_view);
}
