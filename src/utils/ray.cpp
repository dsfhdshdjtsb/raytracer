...
std::vector<Intersection> Ray::intersect(const std::shared_ptr<Shape> shape) const {
    ...
    if(auto group = std::dynamic_pointer_cast<Group>(shape)) { //this is terrible
        ...
        Cube c( bounds.bl, bounds.tr);
        c.transform = group->transform;
        if( c.intersect(*this).size() != 0)  {
            Ray temp = (*this).transform(group->transform.inverse());
            for(auto s : group->shapes)  { 
                std::vector<double> ts = s->intersect(temp);
                for(double t : ts) {
